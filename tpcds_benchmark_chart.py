...
# --- Single chart: speedup only ---
fig, ax = plt.subplots(figsize=(20, 7))
fig.suptitle('TPC-DS SF1 Benchmark: Speedup (morsel-driven parquet vs main)',
             fontsize=16, fontweight='bold', y=0.98)

x = np.arange(len(queries))
colors = ['#55a868' if s > 1.05 else '#c44e52' if s < 0.95 else '#8c8c8c' for s in speedup]
ax.bar(x, speedup, color=colors, width=0.7, alpha=0.85)
ax.axhline(y=1.0, color='black', linewidth=0.8, linestyle='--')
ax.set_ylabel('Speedup (×)', fontsize=12)
ax.set_xlabel('TPC-DS Query', fontsize=12)
ax.set_xticks(x)
ax.set_xticklabels([str(q) for q in queries], fontsize=8, rotation=90)
ax.set_xlim(-0.7, len(queries) - 0.3)
...
for i in top_indices:
    ax.annotate(f'{speedup[i]:.1f}×', xy=(i, speedup[i]),
