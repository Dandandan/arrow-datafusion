...
use datafusion_common::{
    Column, DFSchema, DFSchemaRef, DataFusionError, ExprSchema, Result,
    ScalarValue, plan_err,
};
use datafusion_expr::expr::{self, Exists, SetComparison, SetQuantifier};
use datafusion_expr::logical_plan::{LogicalPlan, Subquery};
...
impl SimplifyExpressions {
    fn optimize_internal(
        ...
        let mut rewrite_expr = |expr: Expr| {
            let expr = expr
                .transform_up(|expr| rewrite_set_comparison(expr, &schema))?
                .data;
