...
impl GroupedHashAggregateStream {
    ...
    fn group_aggregate_batch(&mut self, batch: &RecordBatch) -> Result<()> {
        ...
        for group_values in &group_by_values {
            ...
            // Compute a permutation that sorts group indices for cache locality.
            // Accumulator writes to state arrays (which can be large) become
            // sequential, reducing cache misses. The permutation is passed to
            // accumulators so they can index into values in the sorted order
            // without materializing reordered arrays.
            let opt_permutation = if !self.current_group_indices.is_empty() {
                ...
                Some(self.sort_permutation.as_slice())
            } else {
                None
            };
            let group_indices = &self.current_group_indices;
