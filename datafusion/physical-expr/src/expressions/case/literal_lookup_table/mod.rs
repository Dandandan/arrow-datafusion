...
pub(in super::super) struct LiteralLookupTable {
    ...
    pub(super) lookup: Box<dyn WhenLiteralIndexMap>,

    pub(super) else_index: u32,
    ...
    pub(super) then_and_else_values: ArrayRef,
}

#[derive(Debug)]
pub(in super::super) struct WhenLookupTable {
    /// The lookup table to use for evaluating the CASE expression
    pub(super) lookup: Box<dyn WhenLiteralIndexMap>,

    /// Maps lookup return value (0..unique_literals.len()) to original branch index (0..body.when_then_expr.len())
    pub(super) index_mapping: Vec<u32>,

    pub(super) else_index: u32,
}

impl WhenLookupTable {
    pub(in super::super) fn maybe_new(body: &CaseBody) -> Option<Self> {
        ...
        // Try to downcast all the WHEN expressions to literals
        let when_exprs_maybe_literals = body
            .when_then_expr
            .iter()
            .map(|(when, _)| {
                let when_maybe_literal = when.as_any().downcast_ref::<Literal>();
                when_maybe_literal.map(|l| l.value().clone())
            })
            .collect::<Vec<_>>();

        // If not all the WHEN expressions are literals we cannot use this optimization
        if when_exprs_maybe_literals.iter().any(|v| v.is_none()) {
            return None;
        }

        let when_exprs_scalars = when_exprs_maybe_literals
            .into_iter()
            .flatten()
            // Only keep non-null WHEN literals
            // as they cannot be matched - case NULL WHEN NULL THEN ... ELSE ... END always goes to ELSE
            .enumerate()
            .filter(|(_, when_lit)| !when_lit.is_null())
            .collect::<Vec<_>>();

        if when_exprs_scalars.is_empty() {
        ...
        let mut unique_when = Vec::new();
        let mut index_mapping = Vec::new();
        let mut map = IndexMap::with_capacity(body.when_then_expr.len());

        for (original_index, when) in when_exprs_scalars {
            if !map.contains_key(&when) {
                map.insert(when.clone(), original_index as u32);
                unique_when.push(when);
                index_mapping.push(original_index as u32);
            }
        }

        {
            let when_data_type = unique_when[0].data_type();

            // If not all the WHEN literals are the same data type we cannot use this optimization
            if unique_when.iter().any(|l| l.data_type() != when_data_type) {
                return None;
            }
        }

        let else_index = unique_when.len() as u32;
        let lookup = try_creating_lookup_table(unique_when).ok()?;
