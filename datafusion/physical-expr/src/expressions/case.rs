...
mod tests {
    ...
    #[test]
    fn test_case_with_expr_branch_lookup_dense() -> Result<()> {
        let batch = RecordBatch::try_new(
        ...
        // CASE a WHEN 100 THEN 1 WHEN 101 THEN 2 WHEN 102 THEN 3 ELSE 4 END
        let expr = col("a", &batch.schema())?;
        let case_expr = CaseExpr::try_new(
            Some(expr),
            vec![(lit(100), lit(1)), (lit(101), lit(2)), (lit(102), lit(3))],
            Some(lit(4)),
        )?;

        // Check that it uses WithExprBranchLookupTable (or WithExprScalarLookupTable since it's all literals)
        // Actually since I used literals for THEN as well, it will use WithExprScalarLookupTable
        assert!(matches!(
            case_expr.eval_method,
            EvalMethod::WithExprScalarLookupTable(_)
        ));
